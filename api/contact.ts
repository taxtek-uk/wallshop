...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    // --- Parse body (support raw JSON string or object) ---
    const body =
    ...
    const rawName = safeString(body?.name);
    const rawEmail = safeString(body?.email);
    const rawReason = safeString(body?.reason);
...
<style>
  ...
  .brand-badge { width:44px; height:44px; border-radius:10px; background:${c.brandDark}; color:#fff; display:flex; align-items:center; justify-content:center; font-weight:700; font-size:18px; letter-spacing:0.5px; }
  ...
  <div class="content">
    ...
    <div class="card">
      <table class="kv" role="presentation" aria-label="Contact details">
        ...
        <tr><th scope="row">Subject</th><td><span class="badge">${esc(reason)}</span></td></tr>
      </table>
    </div>
    <div class="card"><div class="message">${esc(message)}</div></div>
    <div class="meta">
      Received: ${esc(timestamp.toLocaleString("en-GB", { weekday:"long", year:"numeric", month:"long", day:"numeric", hour:"2-digit", minute:"2-digit" }))}<br/>
      Source: <span class="foot-strong">thewallshop.co.uk/contact</span>
    </div>
    <div class="actions" style="margin-top:16px;">
      <a class="btn" href="mailto:${esc(email)}?subject=${encodeURIComponent("Re: " + reason + " — The Wall Shop")}">Reply</a>
      <a class="btn secondary" style="margin-left:8px"
         href="mailto:info@thewallshop.co.uk?subject=${encodeURIComponent("Fwd: Contact — " + reason)}&body=${encodeURIComponent(
           `Customer Details:\nName: ${name}\nEmail: ${email}\nSubject: ${reason}\n\nMessage:\n${message}`
         )}">
         Forward internally
      </a>
